...
import { getSalesReturns, createSalesReturn, deleteSalesReturn, getCustomers, getProducts, getSalesOrders } from '@/lib/api';
import type { SalesReturn, Customer, Product, SalesOrder } from '@/types/types';
...
const SalesReturnForm: React.FC<{
  ...
  orders: SalesOrder[];
  userId: string;
  onSave: (r: Parameters<typeof createSalesReturn>[0], items: Parameters<typeof createSalesReturn>[1]) => Promise<void>;
  onClose: () => void;
}> = ({ customers, products, orders, userId, onSave, onClose }) => {
  ...
  const setLine = (i: number, k: string, v: string | number) => {
    setLines(prev => {
      ...
      if (k === 'product_id') {
        ...
        line.unit_price = p?.selling_price ?? 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    await onSave(
      { return_number: `SR-${Date.now()}`, customer_id: customerId || null, sales_order_id: orderId || null, return_date: returnDate, total: totalAmount, reason: reason || null, status: 'draft', created_by: userId },
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div>
          <label className="mb-1 block text-sm font-normal">Original Sales Order</label>
      ...
      <DialogFooter>
        ...
        <button type="submit" disabled={saving} className="rounded bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90 disabled:opacity-60">
          {saving ? 'Saving…' : 'Create Sales Return'}
        </button>
      </DialogFooter>
    </form>
  );
...
export const SalesReturnPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [rRes, cRes, pRes, oRes] = await Promise.all([getSalesReturns(page, PAGE_SIZE), getCustomers(1, 100), getProducts(1, 200), getSalesOrders(1, 100)]);
    setReturns(rRes.data); setTotal(rRes.count); setCustomers(cRes.data); setProducts(pRes.data); setOrders(oRes.data);
    setLoading(false);
  }, [page]);

  useEffect(() => { load(); }, [load]);

  const handleSave = async (r: Parameters<typeof createSalesReturn>[0], items: Parameters<typeof createSalesReturn>[1]) => {
    const { error } = await createSalesReturn(r, items);
    if (error) { toast.error(error.message); return; }
    toast.success('Sales return created'); setFormOpen(false); load();
  };

  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await deleteSalesReturn(deleteId);
    if (error) { toast.error(error.message); return; }
    ...
    <MainLayout>
      <div className="space-y-4">
        ...
        <div className="rounded border border-border bg-card shadow-card overflow-x-auto">
          <table className="w-full min-w-max">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                {['Return #', 'Customer', 'Date', 'Total', 'Status', 'Actions'].map(h => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-muted-foreground">No sales returns yet</td></tr>
              ) : returns.map(r => (
                <tr key={r.id} className="border-b border-border erp-table-row">
                  ...
                  <td className="whitespace-nowrap px-4 py-2.5 text-sm">{r.customers?.name || '—'}</td>
      ...
      <Dialog open={formOpen} onOpenChange={open => { if (!open) setFormOpen(false); }}>
        <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-3xl max-h-[90dvh] overflow-y-auto">
          ...
          <SalesReturnForm customers={customers} products={products} orders={orders} userId={user?.id || ''} onSave={handleSave} onClose={() => setFormOpen(false)} />
        </DialogContent>
      </Dialog>
